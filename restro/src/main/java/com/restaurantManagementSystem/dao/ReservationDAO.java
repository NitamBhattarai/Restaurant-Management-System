...
public class ReservationDAO {
    ...
    public int create(Reservation res) throws SQLException {
        String sql = "INSERT INTO reservations(guest_name, guest_phone, guest_email, "
                   + "table_id, party_size, reserved_at, status, notes) "
                   + "VALUES(?, ?, ?, ?, ?, ?, 'PENDING', ?)";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ps.setString(1, res.getGuestName());
            ps.setString(2, res.getGuestPhone());
            ps.setString(3, res.getGuestEmail());
            if (res.getTableId() != null) ps.setInt(4, res.getTableId());
            else                          ps.setNull(4, Types.INTEGER);
            ps.setInt(5, res.getPartySize());
            ps.setTimestamp(6, res.getReservedAt() != null
                ? Timestamp.valueOf(res.getReservedAt()) : null);
            ps.setString(7, res.getNotes());
            ps.executeUpdate();
    ...
    public boolean update(Reservation res) throws SQLException {
        String sql = "UPDATE reservations SET guest_name=?, guest_phone=?, guest_email=?, "
                   + "table_id=?, party_size=?, reserved_at=?, status=?, notes=? WHERE id=?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, res.getGuestName());
            ps.setString(2, res.getGuestPhone());
    ...
    private Reservation mapRow(ResultSet rs) throws SQLException {
        ...
        if (rat != null) r.setReservedAt(rat.toLocalDateTime());
